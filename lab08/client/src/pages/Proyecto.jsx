...
export default function ProyectosPage() {
  ...
  const validateDates = (inicioFecha, finFecha) => {
    if (!inicioFecha || !finFecha) return true;
    return new Date(inicioFecha) <= new Date(finFecha);
  };

  const handleSubmit = async (e) => {
    e.preventDefault();
    
    if (!validateDates(form.iniFechProy, form.terFechProy)) {
      setErrors({ fechas: 'La fecha de inicio no puede ser posterior a la fecha de fin' });
      return;
    }
      ...
      <div className="max-w-4xl mx-auto px-4">
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {errors.fechas && (
              <div className="text-red-500 text-sm bg-red-50 p-2 rounded">{errors.fechas}</div>
            )}
